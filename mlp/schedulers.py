...
class CosineAnnealingWithWarmRestarts(object):
    ...
    def update_learning_rule(self, learning_rule, epoch_number):
        ...
        total_epochs_per_period = self.total_epochs_per_period
        sum_epoch = total_epochs_per_period
        
        while sum_epoch <= epoch_number:
            max_learning_rate *= self.max_learning_rate_discount_factor
            total_epochs_per_period *= self.period_iteration_expansion_factor
            sum_epoch += total_epochs_per_period
            
        learning_rate = self.min_learning_rate + 0.5 * (max_learning_rate - self.min_learning_rate) *\
                        (1 + np.cos(np.pi * (epoch_number - sum_epoch + total_epochs_per_period) / total_epochs_per_period))
